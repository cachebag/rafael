...
impl RootedFilesystem {
    ...
    pub async fn list_dir(
                ...
                })?
        {
            ...
            let path = entry.path();
            let file_type = entry.file_type().await.map_err(|source| FilesystemError::Io {
                path: path.clone(),
                source,
            })?;
    ...
    async fn write_target(
        ...
        let relative = sanitize_relative_path(path)?;
        let absolute = self.root.join(relative);
        let parent = absolute.parent().ok_or_else(|| FilesystemError::UnsafePath {
            path: path.to_owned(),
...
pub fn is_sensitive_path(path: &str) -> bool {
    ...
    if matches!(file_name.as_str(), ".env.example" | ".env.sample" | ".env.template") {
        return false;
    }
