...
impl GitClient {
        ...
        S: Into<String>,
    {
        Ok(self
            .runner
            .run(self.spec(Some(repo.as_ref()), args)?)
            .await?)
    }
    ...
    pub async fn status_short(&self, repo: impl AsRef<Path>) -> Result<String, GitError> {
        Ok(self.run_required(repo, ["status", "--short"]).await?.stdout)
    }
    ...
    pub async fn rev_parse(&self, repo: impl AsRef<Path>, rev: &str) -> Result<String, GitError> {
        Ok(self
    ...
    pub async fn clone_repo(&self, options: CloneOptions) -> Result<(), GitError> {
        ...
        self.runner
            .run_required(self.spec(options.cwd.as_deref(), args)?)
            .await?;
        Ok(())
    }
